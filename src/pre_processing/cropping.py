...
def crop_image(image, x_start, y_start, x_end, y_end):
    ...
    if len(image.shape) != 3:
        raise ValueError(
            f"Expected a 3D image (Z, Y, X) but got an image with shape {image.shape}"
        )
    ...
    if not (0 <= y_start < y_end <= img_height):
        raise ValueError(
            f"y_start ({y_start}) must be >= 0, < y_end ({y_end}) and <= width ({img_width})"
        )
...
def crop_file(input_path, output_path, x_start, y_start, x_end, y_end):
    ...
    cropped_img = crop_image(img, x_start, y_start, x_end, y_end)
...
def crop_data_set(input_path, output_path, x_start, y_start, x_end, y_end):
    ...
    tasks = [(img_path, output_path, x_start, y_start, x_end, y_end) for img_path in images_path]
