...
def save_results(
    ...
    blob_coord_df[["Zum","Yum","umi"]] = blob_coord_df[["Z","Y","X"]] * resolution

    blob_coord_df.to_csv(
        output_coords_path, index_label="index"
    )
...
def verify_input(vol, channel_id, timepoint, z_min, z_max):
    ...
    if vol.ndim == 5:
        ...
        if timepoint is not None and (timepoint < 0 or timepoint >= t):
            raise ValueError(
                f"Timepoint {timepoint} is out of bounds for volume with {t} timepoints."
            )
    elif vol.ndim == 4:
        z, c, x, y = vol.shape
        if timepoint is not None:
            raise ValueError(f"Timepoint should be None when working with 4D images")
    else:
        raise ValueError(
            f"Input volume must be 5D (T,Z,C,Y,X) or 4D (Z,C,Y,X), but got shape {vol.shape}"
        )
...
def frame_blob_detection(frame, algorithm, threshold, min_sigma, max_sigma, radius_max, area_max):
...
def blob_detection(
    ...
    if vol.ndim == 4:
        print(f"Processing the single frame")
        timepoint = 0
        z, c, x, y = vol.shape
        ...
        # Process for all the frames
            ...
            for ti in tqdm(range(t), desc="Detecting blobs in frames", unit="frame"):
                [blobs_coord, composite[ti]] = frame_blob_detection(
                    vol_c[ti],
                    algorithm,
                    threshold,
                    min_sigma,
    ...
    resolution_px_to_um = np.array(resolution) / np.array([z,y,x])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--min_sigma", nargs="?", type=float, default = 1.0)
    parser.add_argument("--max_sigma", nargs="?", type=float, default = 3.0)
